...
def forward_checking(state, verbose=False):
    ...
    cur_name = state.get_current_variable_name()
    X = state.get_variable_by_name(cur_name)
    constraints = state.get_constraints_by_name(cur_name)
    for c in constraints:
        Y = state.get_variable_by_name(c.get_variable_j_name())
        for y in Y.get_domain():
            if not c.check(state,X.get_assigned_value(),y):
                Y.reduce_domain(y)
...
def forward_checking_prop_singleton(state, verbose=False):
    ...
    queue = [v for v in state.get_all_variables() if v.domain_size()==1]
    visited = []
    while queue:
        v = queue.pop(0)
        visited.append(v.get_name())
        constraints = state.get_constraints_by_name(v.get_name())
        for c in constraints:
            Y = state.get_variable_by_name(c.get_variable_j_name())
            for y in Y.get_domain():
                if not c.check(state,v.get_domain()[0],y):
                    Y.reduce_domain(y)
                if len(Y.get_domain())==0:
                    return False
        for var in state.get_all_variables():
            if var not in queue and var.get_name() not in visited and var.domain_size()==1:
                queue.append(var)
    return True
...
def euclidean_distance(list1, list2):
    ...
    return sum([(list1[i]-list2[i])**2 for i in range(len(list1))])**.5
...
def information_disorder(yes, no):
    ...
    def disorder(l):
        ...
        b = float(len(l))
        for c in classes:
            d-=l.count(c)/b*math.log(l.count(c)/b,2)
        return d
    return p/t*disorder(yes)+n/t*disorder(no)
...
WHAT_I_FOUND_INTERESTING = "forward checking code"
WHAT_I_FOUND_BORING = "implementing functions"
