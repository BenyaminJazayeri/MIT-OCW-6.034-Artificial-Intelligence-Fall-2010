...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        """
        res = 0
        for c in self.classifires:
            res += c[0].classify(obj)*c[1]
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        for i in range(len(self.data_weights)):
            ...
            if best_classifier.classify(data) == self.standard.classify(data):
                self.data_weights[i] *= (.5*1/(1-best_error))
            else:
                self.data_weights[i] = 0.5 * (1 / best_error) * self.data_weights[i]
