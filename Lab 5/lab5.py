...
def most_misclassified(classifier, n=5):
    ...
    """
    weights = classifier.data_weights
    data = classifier.data

    combined_list = zip(weights, data)
    scl = sorted(combined_list, key=lambda cl: cl[0], reverse=True)
    del scl[n:]

    return [legislator_info(l[1]) for l in scl]
