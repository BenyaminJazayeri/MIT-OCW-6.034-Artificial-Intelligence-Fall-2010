...
class Neuron(DifferentiableElement):
    ...
    def compute_doutdx(self, elem):
        ...
        if self.has_weight(elem):
            return o * (1-o) * self.get_inputs()[self.get_weights().index(elem)].output()
        d = 0
        for w in self.get_weights():
            if self.isa_descendant_weight_of(elem,w):
                d += w.get_value() * self.my_inputs[self.my_weights.index(w)].dOutdX(elem)
        return self.compute_output() * (1 - self.compute_output()) * d
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        return -0.5*(self.get_input().compute_output()-self.my_desired_val)**2
    ...
    def dOutdX(self, elem):
        ...
        """
        return (self.my_desired_val - self.get_input().compute_output()) * self.get_input().dOutdX(elem)
...
def make_neural_net_challenging():
    ...
    wDF = Weight('wEF', random_weight())
    wEF  = Weight('wDF', random_weight())
    wF  = Weight('wF', random_weight())
    ...
    E = Neuron('E', [A,B,C,i0], [wAE,wBE,wCE,wE])

    F = Neuron('F', [D,E,i0], [wDF,wEF,wF])
    
    P = PerformanceElem(F, 0.0)

    net = Network(P,[A,B,C,D,E,F])
    return net
